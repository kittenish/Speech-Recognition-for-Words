...
%                      6  q.sl  min slope at threshold (must be >0) [0.02]
...
%      Version: $Id: psycest.m 7828 2016-04-24 17:21:23Z dmb $
%
...
if iq<0  % initialization
    ...
    nr=[40 21 30 1 0.2 0.02 4 1.3 2 1 1 1 0.0001 2 10 0.5 0.1 1 0.01]';                      % default parameter values
    nrf={'nx','ns','nh','cs','dh','sl','kp','hg','cf','pm','lg','pp', 'pf', 'ts', 'dp', 'it','at','la','op'};     % parameter field names
    numnr=length(nr);
...
elseif iq>0
    ...
    if  nresq(iq)<2              % keep old limits if nresq(iq)<2
        ...
        xq2lim=[mqi(1)-nr(7)*xsd, mqi(1)+nr(7)*xsd];
        sq2lim=[max(sqmin,mqi(2)-nr(7)*ssd),mqi(2)+nr(7)*ssd];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            switch poi
                ...
                case 'x' % plot probe choice
                    ...
                    if nr(18) % if doing look 2-ahead
                        ...
                        colorbar;
                        hold on
                        [hnmin,ii]=max(hn);         % find teh next model to probe
